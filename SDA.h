...
class SDA {
 public:
  SDA();
  SDA(const SDA &other);
  // The equals sign after the arguments provides the default value for the
  // parameters. explicit SDA(int numStates, int numChars, int maxRespLen, int
  // outputLen, int initState = 0, bool verbose = false);
 ...
 private:
  ...
  /**
   * The transition vectors for each state.  For example transitions[3][1] = 5
   * means the SDA will transition from state 3 to state 5 if a 1 is read.
   */
  vector<vector<int>> transitions;
  /**
   * The response vectors for each transition.  For example responses[3][1] =
   * {1,0} means the SDA will append 0 and 1 to output if a 1 is read at
   * state 3.
   */
